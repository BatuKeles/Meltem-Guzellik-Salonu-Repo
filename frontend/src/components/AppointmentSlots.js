...
//         const res = await networkLayer.get(`https://localhost:5000/api/appointments?date=${formattedDate}`);
      ...
      try {
        ...
        res.data.forEach(({ startTime, endTime }) => {
          ...
          while (hour < endHour || (hour === endHour && minute < endMinute)) {
            ...
            const nextStr = `${String(hour).padStart(2, '0')}:${String(minute).padStart(2, '0')}`;
            taken.push(`${currentStr} - ${nextStr}`);
          }
        });
